            ...
            while True:
                ...
                try:
                    ...
                    if buffer and detector.should_wait_for_input(buffer, time_since_last):
                        # This looks like a real prompt
                        prompt_line = buffer.strip()
                        detector.seen_prompts.add(prompt_line)
                        # Check if this is the multi-line hypothesis prompt
                        if "press enter twice when done" in prompt_line.lower():
                            process.stdin.write((user_input + '\n\n').encode()) 
...
def minimal_input_handler(prompt: str) -> str:
    """
    Minimal handler that just gets input
    """
    return input().strip()
...
async def run_mas_interactive():
    ...
    result = await launch_mas_interactive(run_id, job, minimal_input_handler)
