...
const contractLabel = (s) =>
  isHexAddr(s) ? shortAddr(s) : String(s || "Contract");
...
export default function Diagram({ runId: runIdProp }) {
      ...
      if (idx < prevSum + cap) {
        ...
        const maxR = Math.max(
          ...
          Math.min(
            center.x - pad,
            center.y - pad,
            containerRect.width - center.x - pad,
            containerRect.height - center.y - pad
          )
        );
  ...
  const ensureContractNode = (contractStr) => {
    ...
    node = cy.add({
      ...
      data: {
        id,
        label: contractLabel(contractStr),
  ...
  useEffect(() => {
    ...
    const onMessage = async (evt) => {
      ...
      if (t === "executor-tool-call") {
        ...
        const toolName =
          ...
          (typeof msg.data?.content === "string"
            ? (() => {
                ...
                try {
                  return JSON.parse(cleaned)?.tool_name || "";
                } catch {
                  return (
                    cleaned.match(/"tool_name"\s*:\s*"([^"]+)"/)?.[1] || ""
                  );
                }
              })()
            : "") ||
        ...
        else if (Array.isArray(msg.data?.args?.addresses))
          contracts = msg.data.args.addresses;
  ...
  return (
    <div
      className="w-full h-full bg-surface bg-[radial-gradient(circle,_#1e293b_1px,_transparent_1px)] [background-size:20px_20px] rounded-lg shadow-md"
      ref={containerRef}
    />
  );
}
